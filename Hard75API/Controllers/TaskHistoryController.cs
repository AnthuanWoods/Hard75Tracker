...
namespace Hard75API.Controllers
{
    ...
    public class TaskHistoryController
    {
        ...
        public TaskHistoryController(IConfiguration configuration)
        {
        ...
        [HttpGet]
        [Route("GetTasks")]
        public string GetTaskHistoryDay(string userID, DateOnly dtq)
        {
            Log.Information("Get TaskHistory");
            Hard75Shared.Response response = new Hard75Shared.Response();
            try
            {
                Log.Information("GetTaskHistoryList: Opening DB Connection");
                using (SqlConnection con = new SqlConnection(_configuration.GetConnectionString("DatabaseConnectionString").ToString()))
                {
                    con.Open();
                    Log.Information("GetTaskHistoryList: Selecting all tasks for the day");
                    using (SqlCommand sqlcmd2 = new SqlCommand("Select t.taskID,t.taskName,th.dtComplete from Tasks t left join TaskHistory th on t.taskid = th.taskid and " +
                        " where userID = @userID and dtGoal = @dtQuery", con))
                    {
                        sqlcmd2.Parameters.AddWithValue("@userID", userID);
                        sqlcmd2.Parameters.AddWithValue("@dtQuery", dtq);
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            Log.Information("Found TaskHistory");
                            ...
                            foreach (DataRow record in dt.Rows)
                            {
                                ...
                                if (record[2]  != null)
                                {
                                    task.daycomplete = true;
                                }
                                else
                                {
                                    task.daycomplete= false;
                                }
                                tasks.Add(task);
                            }
            ...
            catch (Exception ex)
            {
                ...
                response.message = "Select TaskHistory Error:" + ex.ToString();
                return JsonConvert.SerializeObject(response);
            }
        }
        ...
        {
            Log.Information("CreateTaskHistory: Started");
            Hard75Shared.Response response = new Hard75Shared.Response();
            try
            {
                Log.Information("CreateTaskHistory: Opening Connection");
                using (SqlConnection con = new SqlConnection(_configuration.GetConnectionString("DatabaseConnectionString").ToString()))
                {
                    con.Open();
                    Log.Information("CreateTaskHistory: Running Query");
                    using (SqlCommand sqlcmd2 = new SqlCommand("Insert into TaskHistory(taskID, userID, dtGoal) Values (@taskID, @userID, @dtGoal)", con))
                    {
                        sqlcmd2.Parameters.AddWithValue("@taskID", task.taskID);
                        sqlcmd2.Parameters.AddWithValue("@dtGoal", DateTime.UtcNow);
